...
export const command = {
  ...
  handler: (args: string[], input: SlashCommandInput): string => {
    if (args.length > 0 && args[0] === "-m" && args.length >= 2) {
      ...
      const result = spawnSync("git", ["commit", "-m", message], { cwd: input.cwd, encoding: "utf8" });
      if (result.status === 0) {
    ...
    try {
      const staged = execSync("git diff --cached --stat", { cwd: input.cwd, encoding: "utf8", stdio: "pipe" });
      const status = execSync("git status --short", { cwd: input.cwd, encoding: "utf8", stdio: "pipe" });
      return [
        "Staged changes:",
        staged.trim() || "  (none)",
        "",
