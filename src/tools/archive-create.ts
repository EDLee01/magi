...
export interface ArchiveCreateResult { path: string; format: string; sizeBytes: number; fileCount: number }
export const ArchiveCreateInputSchema = { type: "object", properties: { source: { type: "string" }, output: { type: "string" }, format: { type: "string", enum: ["tar.gz", "zip"] } }, required: ["source", "output"], additionalProperties: false } satisfies Record<string, unknown>;

export function parseArchiveCreateInput(input: Record<string, unknown>): { source: string; output: string; format: string } {
  return {
...
export function executeArchiveCreate(input: { source: string; output: string; format: string; cwd: string }): ArchiveCreateResult {
