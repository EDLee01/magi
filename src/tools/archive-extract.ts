...
export interface ArchiveExtractResult { path: string; outputDir: string; succeeded: boolean }
export const ArchiveExtractInputSchema = { type: "object", properties: { archive: { type: "string" }, output: { type: "string" } }, required: ["archive"], additionalProperties: false } satisfies Record<string, unknown>;

export function parseArchiveExtractInput(input: Record<string, unknown>): { archive: string; output?: string } {
  return {
...
export function executeArchiveExtract(input: { archive: string; output?: string; cwd: string }): ArchiveExtractResult {
