...
export interface FileDeleteResult { path: string; sizeBytes: number; directories: number; files: number }

export const FileDeleteInputSchema = { type: "object", properties: { path: { type: "string" }, recursive: { type: "boolean" }, force: { type: "boolean" } }, required: ["path"], additionalProperties: false } satisfies Record<string, unknown>;

export function parseFileDeleteInput(input: Record<string, unknown>): { path: string; recursive: boolean; force: boolean } {
...
export function executeFileDelete(input: { path: string; recursive: boolean; force: boolean; cwd: string }): FileDeleteResult {
