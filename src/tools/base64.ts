...
export interface Base64Result { action: string; output: string }
export const Base64InputSchema = { type: "object", properties: { action: { type: "string", enum: ["encode", "decode"] }, text: { type: "string" } }, required: ["action", "text"], additionalProperties: false } satisfies Record<string, unknown>;

export function parseBase64Input(input: Record<string, unknown>): { action: "encode" | "decode"; text: string } {
