...
describe("hooks", () => {
  it("accepts expanded hook events and stronger matcher conditions", () => {
    ...
    expect(matchesHookCondition("agentType:worker", {
      sessionId: "s1",
      cwd: workspace,
      agentType: "worker"
    })).toBe(true);
    expect(matchesHookCondition("*.ts", {
      sessionId: "s1",
      cwd: workspace,
      filePath: "src/main.ts"
    })).toBe(true);
    expect(matchesHookCondition("Config(context.*)", {
      sessionId: "s1",
      cwd: workspace,
      toolName: "Config",
  ...
  it("records session lifecycle hooks and hook_result events through QueryEngine", async () => {
    ...
    try {
      ...
      const audits = store.listAuditEvents(20).filter((event) => event.action === "agent.hook.completed");
      ...
      expect(audits.map((event) => event.target)).toEqual(["session_end:command", "session_start:command"]);
    } finally {
      store.close();
    }
  });
  ...
  it("runs user prompt and permission-denied hooks through QueryEngine", async () => {
    ...
    try {
      ...
      const engine = new QueryEngine({
        ...
        hooks: [
          { event: "user_prompt_submit", type: "command", command: "node -e 'process.stdout.write(process.env.ARGUMENTS)'" },
          { event: "permission_denied", type: "command", if: "FileWrite(*)", command: "node -e 'process.stdout.write(process.env.ARGUMENTS)'" }
        ]
      });

      const result = await engine.submitMessage("please update config");
      const hookEvents = result.events.filter((event) => event.type === "hook_result");
      expect(hookEvents.map((event) => event.event)).toEqual(expect.arrayContaining([
        "user_prompt_submit",
        "permission_denied"
      ]));
      expect(hookEvents.some((event) => event.type === "hook_result" && event.result.output.includes("please update config"))).toBe(true);
      expect(store.listAuditEvents(30).map((event) => event.action)).toEqual(expect.arrayContaining([
  ...
  it("runs config-change hooks for approved Config updates through QueryEngine", async () => {
    ...
    try {
      ...
      const engine = new QueryEngine({
        ...
        hooks: [
          { event: "config_change", type: "command", if: "Config(context.*)", command: "printf config:$ARGUMENTS" }
        ]
      });
      ...
      expect(result.events).toContainEqual(expect.objectContaining({
        type: "hook_result",
        event: "config_change",
        result: expect.objectContaining({ output: expect.stringContaining("context.recentMessages") })
      }));
      expect(store.listAuditEvents(30).map((event) => event.action)).toContain("agent.config.updated");
    } finally {
      store.close();
    }
  });
  ...
  it("runs prompt hooks through an explicit model runner", async () => {
    ...
    const results = await executeHooks({
      ...
      promptModel: async ({ model, messages }) => {
        seen.push({ model, text: messages[0].content[0].type === "text" ? messages[0].content[0].text : "" });
  ...
  it("runs prompt hooks from QueryEngine session lifecycle hooks", async () => {
    ...
    try {
      ...
      const result = await engine.submitMessage("hello");
      const hookEvent = result.events.find((event) => event.type === "hook_result" && event.event === "session_end");
