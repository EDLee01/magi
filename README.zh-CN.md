...
从 v0.1.14 开始，在任务进程运行期间，操作审批和用户问题会保留到你回答或取消。待审批的操作保持暂停，等待时间再长也不会自动获得授权。取消任务或关闭进程会结束待处理的交互。交互状态保存在内存中，重启后需要重新发起确认。

`MAGI_INTERACTION_TIMEOUT_MS` 同时控制 TUI 和控制服务的交互等待：

- 未设置或设为 `0`：持续等待回答或取消，默认采用此行为。
- 设为 `1` 至 `2147483647` 的整数：经过指定毫秒数后超时。

例如，`MAGI_INTERACTION_TIMEOUT_MS=86400000 magi` 将等待时间设为 24 小时。模型请求与 Shell 命令的超时由各自的配置控制。
