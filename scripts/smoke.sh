...
[[ -n "$MSG" ]] && pass "fast 返回: $MSG" || fail "fast 无输出"

header "2. 工具定义 — main 模型带 85 个工具"
...
[[ -n "$MSG" ]] && pass "main 返回: $MSG" || fail "main 无输出"

header "3. 工具调用 — 创建文件 + 读取"
cd "$TMP"
RESULT=$(node "$CLI" -p "Create hello.txt with content world, then read it and tell me the content" -m main --permission-mode acceptEdits 2>/dev/null | head -1)
[[ "$RESULT" == *"world"* ]] && pass "write+read 返回: $RESULT" || fail "工具调用失败: $RESULT"

header "4. Bash 工具 — 运行命令"
RESULT=$(node "$CLI" -p "run bash echo hello-smoke" -m main --permission-mode acceptEdits 2>/dev/null | head -1)
[[ -n "$RESULT" ]] && pass "bash 返回: $RESULT" || fail "bash 失败"
...
[[ "$MSG" == "BAD" ]] && fail "fast 无声无息" || pass "a 返回了内容"
